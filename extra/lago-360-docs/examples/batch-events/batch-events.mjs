 ...
 */
import { selectBatchOutcome, validateBatchRequest } from "./batch-contract.mjs";
...
async function sendBatch(batch) {
  ...
  if (!response.ok)
    throw new Error(
      `Lago respondeu ${response.status}: ${JSON.stringify(body)}`
    );
  return body;
}

const payload = validateBatchRequest({ events });
const result = await sendBatch(payload.events);
const { accepted, failures, retry } = selectBatchOutcome(
  payload.events,
  result.errors
);
...
console.log(
  JSON.stringify(
    {
      accepted: accepted.map(({ transaction_id }) => transaction_id),
      failures,
    },
    null,
    2
...
if (retry.length) {
  console.log(
    "Corrija apenas estes eventos antes de reenviar:",
    retry.map(({ transaction_id }) => transaction_id).join(", ")
  );
}
