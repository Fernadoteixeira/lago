    ...
    <div className="atlas-app">
      <aside className="atlas-sidebar" aria-label="Navegação da documentação">
        <div className="brand-lockup" role="banner">
          ...
          <div><span className="brand-name">Lago</span><span className="brand-sub">360º Billing Atlas</span></div>
        </div>

        <nav className="sidebar-nav" aria-label="Seções">
          {[
            ["overview", "Visão geral", Network],
            ["batch", "Batch events", Activity],
      ...
      <main className="atlas-main">
            ...
            <div className="model-toggle" role="tablist" aria-label="Modelo de cobrança">
              {(["graduated", "volume"] as ChargeModel[]).map((item) => (
                <button key={item} className={model === item ? "is-active" : ""} onClick={() => setModel(item)}>
                  <span>{item === "graduated" ? "Graduated" : "Volume"}</span>
                  <small>{item === "graduated" ? "incrementos por faixa" : "uma tarifa para o total"}</small>
                </button>
              ))}
        ...
        <footer className="atlas-footer"><span>ATLAS DE OPERAÇÃO · LAGO BILLING</span><span>Baseado no OpenAPI 1.51.0 · simulações explicitamente didáticas</span></footer>
      </main>
    </div>
  );
}
